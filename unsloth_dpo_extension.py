...
class UnslothModel(BaseModel):
    ...
    def train_dpo(
        ...
        **kwargs,
        ...
        Args:
            train_dataset: DPO 포맷 데이터셋 (prompt, chosen, rejected)
            eval_dataset: 평가용 데이터셋 (Optional)
            **kwargs: DPO 학습 설정
        """
        FastLanguageModel.for_training(self.model)
        ...
        # DPO 학습 설정
        dpo_args = DPOConfig(
            output_dir=kwargs.get("output_dir", "./output_dpo"),
            ...
            # DPO 특화 파라미터
            beta=kwargs.get("beta", 0.1),  # DPO loss의 온도 파라미터
            max_prompt_length=kwargs.get("max_prompt_length", self.max_seq_length // 2),
            max_length=kwargs.get("max_length", self.max_seq_length),
        )
        ...
        print(f"  - Max Prompt Length: {dpo_args.max_prompt_length}")
        print(f"  - Max Length: {dpo_args.max_length}")
        print(f"  - Learning Rate: {dpo_args.learning_rate}")
        print(f"  - Batch Size: {dpo_args.per_device_train_batch_size}")
        print(f"  - Gradient Accumulation: {dpo_args.gradient_accumulation_steps}")
        print(f"  - Epochs: {dpo_args.num_train_epochs}")
        print(f"{'='*60}\n")
        ...
        dpo_trainer = DPOTrainer(
            ...
            args=dpo_args,
            train_dataset=train_dataset,
            eval_dataset=None if dpo_args.eval_strategy == "no" else eval_dataset,
            processing_class=self.tokenizer,
        )
        ...
        # 모델 저장
        if kwargs.get("save_model", True):
            save_path = kwargs.get("output_dir", "./output_dpo")
            self.save_model(save_path)
            print(f"\n✅ DPO 학습 완료! 모델 저장 위치: {save_path}")
