...
"""
CSV 파일 비교 분석 스크립트
원본 train.csv와 combined.csv의 차이점 분석
"""
...
def compare_csv_files(original_path: str, combined_path: str) -> None:
    ...
    print(f"  비교 대상 ({combined_path}): {len(combined)}개 데이터")
    print(f"  차이: {len(original) - len(combined)}개")

    # 컬럼 비교
    print(f"\n[2] 컬럼 비교")
    print(f"  원본 컬럼: {list(original.columns)}")
    print(f"  비교 대상 컬럼: {list(combined.columns)}")

    # ID 비교
    print(f"\n[3] ID 비교")
    original_ids = set(original["id"].tolist())
    combined_ids = set(combined["id"].tolist())
    ...
    combined_duplicates = combined[combined["id"].duplicated(keep=False)]
    ...
    if len(combined_duplicates) > 0:
        ...
        for id_val in combined_duplicates["id"].unique()[:10]:
            count = len(combined[combined["id"] == id_val])
            print(f"    - {id_val}: {count}번 중복")
    ...
    for id_val in list(common_ids)[:100]:  # 처음 100개만 확인
        ...
        comb_row = combined[combined["id"] == id_val].iloc[0]
                        ...
                        diff_examples.append(
                            {
                                ...
                                "combined": comb_val[:100] + "..." if len(comb_val) > 100 else comb_val,
                            }
                        )
