...
# Unsloth의 untrained tokens 경고 무시
os.environ["UNSLOTH_IGNORE_WARNINGS"] = "1"
warnings.filterwarnings("ignore", message=".*Untrained tokens.*")

from src.model.base_model import BaseModel
from src.utils.registry import MODEL_REGISTRY

@MODEL_REGISTRY.register("qwen_base")
class UnslothModel(BaseModel):
    ...
    def __init__(self, model_name_or_path: str, **kwargs):
        ...
        # ✅ Qwen 모델의 chat_template 설정 (한국 역사 문제 답변용)
        if self.tokenizer.chat_template is None:
            # 프로젝트 특화: 지문을 읽고 1-5번 중 답을 고르는 형태
    ...
    def train(
        ...
        **kwargs,
        ...
        # 🔧 Unsloth의 fix_untrained_tokens 검사 우회 (Base 모델용)
        import unsloth_zoo.tokenizer_utils as tokenizer_utils
        original_fix_untrained_tokens = tokenizer_utils.fix_untrained_tokens
        
        def dummy_fix_untrained_tokens(*args, **kwargs):
            """Base 모델에서 untrained tokens 경고를 무시"""
            print("⚠️ Unsloth untrained tokens 검사를 우회합니다 (Base 모델 사용 중)")
            pass
        
        # Monkey patch 적용
        tokenizer_utils.fix_untrained_tokens = dummy_fix_untrained_tokens
        ...
        try:
            ...
            # 원래 함수로 복원
            tokenizer_utils.fix_untrained_tokens = original_fix_untrained_tokens
