...
@hydra.main(version_base=None, config_path=str(project_root / "config"), config_name="qwen2.5")
def main(cfg: DictConfig):
    ...
    if cfg.mode == "train":
        ...
        # 학습 타입 확인: SFT vs DPO
        # ===================================
        train_type = cfg.get("train_type", "sft")  # 기본값은 SFT
        
        print(f"\n{'='*70}")
        print(f"🎯 학습 타입: {train_type.upper()}")
        print(f"{'='*70}\n")
        ...
        if train_type == "dpo":
            ...
            # DPO 데이터셋 타입과 경로 확인
            dpo_dataset_type = cfg.dataset.get("dpo_type", "dpo")
            dpo_dataset_path = cfg.dataset.get("dpo_path", "data/dpo_train.csv")
            
            if not os.path.exists(dpo_dataset_path):
                raise ValueError(f"DPO 데이터셋 경로를 찾을 수 없습니다: {dpo_dataset_path}")
            
            # DPO 데이터셋 로드
            dataset_cls = DATASET_REGISTRY.get(dpo_dataset_type)
            dataset = dataset_cls(dpo_dataset_path)
            
            # DPO 전처리 (prompt, chosen, rejected 형태로 변환)
            processed_dataset = dataset.preprocess(
                tokenizer,
                max_length=cfg.model.max_seq_length,
                max_prompt_length=cfg.dpo.get("max_prompt_length", cfg.model.max_seq_length // 2),
                # **cfg.dataset.preprocess.get("dpo", {}) # dpo에서 전처리가 없으므로 주석처
            )
            ...
            dpo_config = {
                **cfg.training,
                **cfg.dpo  # DPO 전용 설정 추가
            }
            
            # DPO 학습 실행
            model.train_dpo(
                train_dataset=processed_dataset["train"],
                eval_dataset=None,  # DPO는 일반적으로 eval 없이 학습
                **dpo_config
            )
